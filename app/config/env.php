...
if (DATABASE_URL) {
    ...
    define('DB_HOST', $dbUrl['host'] ?? $_ENV['DB_HOST'] ?? 'localhost');
    define('DB_PORT', $dbUrl['port'] ?? $_ENV['DB_PORT'] ?? '5432');
    define('DB_USER', $dbUrl['user'] ?? $_ENV['DB_USER'] ?? 'user');
    define('DB_PASSWORD', $dbUrl['pass'] ?? $_ENV['DB_PASSWORD'] ?? 'password');
    define('DB_NAME', trim($dbUrl['path'] ?? '', '/') ?? $_ENV['DB_NAME'] ?? 'dbname');
} else {
